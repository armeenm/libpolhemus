...
int DevHandle::Impl::transfer_raw(Buffer* buf, unsigned char ep) const noexcept {
  int transferred;
...
int DevHandle::Impl::send_buf(const Buffer& cmd, Buffer* resp, bool add_cr) const noexcept {
...
int DevHandle::Impl::recv_raw(Buffer* buf) const noexcept { return transfer_raw(buf, info.read_ep); }
